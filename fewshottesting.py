...
# This is the testing & evaluation suite for the two models (reptile and standard).
...
# of subject.
# The aim is that the Reptile algorithm will adapt and generalise better than the standard model.
...
for subj in range(1, 12):
    ...
    reptile_model = tf.keras.models.load_model(f'{MODEL_DIRECTORY}/reptile_model_v1.h5')

    print(f"Testing Reptile Model on Subject {subj}...")
    ...
    model = tf.keras.models.load_model(F'{MODEL_DIRECTORY}/standard_cnn_model_v1.h5')

    print(f"Testing Standard Model on Subject {subj}...")
    ...
    print('\n')
    print(f'Reptile subject {subj} accuracy = {reptile_result}%')
    ...
    output_list.append(f'Reptile Model Accuracy: {reptile_result}%')
    output_list.append(f'Standard Model Accuracy: {result}%') 
    output_list.append("\n")


print(f'\nReptile achieved higher accuracy than the standard model on {wincount}/11 occasions\n')
output_list.append(f'\nReptile achieved higher accuracy than the standard model on {wincount}/11 occasions')

# Log the test to a txt file
current_time = datetime.datetime.now()
timestamp = current_time.strftime("%d-%m-%Y_%H-%M")
with open(f'{MODEL_DIRECTORY}/amputee_testing/DB3_test_{timestamp}.txt', 'w') as f:
