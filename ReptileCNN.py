...
def load_checkpoint():
    ...
    model = tf.keras.models.load_model(f'{MODEL_DIRECTORY}/checkpoint_reptile_model.h5')
    
    # Load the last iteration
    with open(f"{MODEL_DIRECTORY}\\current_iteration.txt", "r") as f:
...
def reptile(model_fn, train_fn, tasks, num_iterations, k, epsilon):
    ...
    for iteration in range(current_iter, num_iterations):
        ...
        # Save metric lists to be opened again later and examined by iterating over dictionary and write each list to separate text file
        for name, lst in lists.items():
            with open(f"{MODEL_DIRECTORY}\\{name}.txt", "a") as f:
                for item in lst:
                    f.write(f"{item}\n")

        # Clear lists at this iteration once they've been appended to file to save memory
        ...
        with open(f"{MODEL_DIRECTORY}/current_iteration.txt", "w") as f:
        ...
        try:
            ...
            model.save(f'{MODEL_DIRECTORY}/checkpoint_reptile_model.h5')               
        except:
            ...
            for filename in files:
                if indic in filename:
                    ...
                    if float(accuracy) >= oldacc:
                        ...
                        model.save(f'{MODEL_DIRECTORY}/checkpoint_reptile_model_best_{straccuracy}.h5')
                    else:
                        pass
...
# Allow user to select to continue previous training or start fresh
...
if loadyesno.lower() == 'load':
    ...
    # Final model save
    model.save(f'{MODEL_DIRECTORY}/reptile_model.h5')
else:
    # Remove the current metric savefiles if they exist
    try:
        os.remove(f'{MODEL_DIRECTORY}/test_accuracy_list.txt')
        os.remove(f'{MODEL_DIRECTORY}/test_loss_list.txt')
        os.remove(f'{MODEL_DIRECTORY}/task_train_accuracy_list.txt')
        os.remove(f'{MODEL_DIRECTORY}/task_train_loss_list.txt')   
        os.remove(f'{MODEL_DIRECTORY}/task_test_accuracy_list.txt')     
        os.remove(f'{MODEL_DIRECTORY}/task_test_loss_list.txt')
    except:
        pass
...
full_test_accuracy_list = read_file_to_list(os.path.join(MODEL_DIRECTORY, 'test_accuracy_list.txt'))
full_test_accuracy_list = [float(x.strip('[]')) for x in full_test_accuracy_list]

full_test_loss_list = read_file_to_list(os.path.join(MODEL_DIRECTORY, 'test_loss_list.txt'))
full_test_loss_list = [float(x.strip('[]')) for x in full_test_loss_list]

full_task_train_accuracy_list = read_file_to_list(os.path.join(MODEL_DIRECTORY, 'task_train_accuracy_list.txt'))
full_task_train_accuracy_list = [float(x.strip('[]')) for x in full_task_train_accuracy_list]

full_task_train_loss_list = read_file_to_list(os.path.join(MODEL_DIRECTORY, 'task_train_loss_list.txt'))
full_task_train_loss_list = [float(x.strip('[]')) for x in full_task_train_loss_list]

full_task_test_accuracy_list = read_file_to_list(os.path.join(MODEL_DIRECTORY, 'task_test_accuracy_list.txt'))
full_task_test_accuracy_list = [float(x.strip('[]')) for x in full_task_test_accuracy_list]

full_task_test_loss_list = read_file_to_list(os.path.join(MODEL_DIRECTORY, 'task_test_loss_list.txt'))
